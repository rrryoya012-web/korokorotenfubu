...
### サイズ再調整
- **幅の拡大**:
    - 本家のプレイ感に近づけるため、箱の幅を 333px → 380px に変更。
    - これによりアスペクト比が約 4:5 になり、窮屈感が解消される。
- **画像のリサイズ**:
    - 箱のサイズ変更に伴い、背景画像 `bg01.png` を 333px × 500px から 380px × 500px にリサイズ（引き伸ばし）。

### UI配置変更
- **SCORE/NEXT**: 画面左上から右下へ移動。
- **進化ルート**: 画面右側から左側へ移動。によるバランス調整。

### レスポンシブ対応
- **画面リサイズ機能**:
    - `resizeCanvas()` を実装し、ウィンドウサイズに合わせてCanvasを自動拡大縮小。
    - アスペクト比を維持しつつ、画面いっぱいに表示。
- **スマホ操作対応**:
    - `touchmove`, `touchend` イベントを追加し、タップ＆スワイプで操作可能に。
    - CSS `touch-action: none` で誤操作（スクロール）を防止。
    - 入力座標の変換処理を追加し、リサイズ時でも正確な操作を実現。
- **UI動的配置**:
    - `updateUIPositions()` を実装し、Canvas基準でUI要素を配置。
    - SCORE/NEXTはCanvas右下、歯車は左下、進化ルートはCanvas左側。
    - ウィンドウリサイズ時にも追従。
